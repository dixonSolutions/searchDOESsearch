...
restart_hint() {
  ...
  echo "    Log out and back in, OR test the change in a nested session:"
  echo -e "    ${CYAN}./scripts/nested-test.sh${RESET}   (add --headless for no window)"
  echo "    (needs mutter-dev-bin; --nested was removed in Shell 49+)"
  echo ""
